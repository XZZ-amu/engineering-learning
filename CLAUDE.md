...
### P0：需要深入的
1. **API 设计** — RESTful 规范、请求/响应结构、错误码、版本管理 (#1)
2. **数据库基础** — SQL vs NoSQL、表结构设计、查询、索引 (#2)
3. **网络协议** — HTTP 完整过程、状态码、缓存机制 (#3)
4. **认证与权限** — 登录机制、Token/Session、OAuth、RBAC (#4)

### P1：有余力时
5. **前端基础** — React 组件的工作方式、状态管理、数据流（因为桌面端在用）
6. **性能优化** — 前端性能、后端性能、常见瓶颈
7. **安全基础** — XSS、CSRF、SQL 注入（知道是什么就行）
8. **系统设计入门** — 怎么从零设计一个系统的思路
